...
class Database {
    ...
    tableCheckCreation() { // methode to create all the required mysql tables
        ...
        this.connection().query(`CREATE TABLE IF NOT EXISTS dc_private_channels (channel_id VARCHAR(30), guild_id VARCHAR(30), role_id VARCHAR(30), renewal_in_ms VARCHAR(255), added_at VARCHAR(30), added_by VARCHAR(30))`, (err) => {
            if(err) throw err;
        });
